...
def extract_rankings_from_html(html, keyword, expected_len):
    """
    HTMLからランキング情報を抽出し、指定された expected_len の長さのリストで返す。
    """
    rankings = []
...
def get_rankings_from_url(url, keyword, expected_len):
    """
    URLからHTMLを取得し、指定された expected_len 分のランキングを返す。
    """
...
# --- 以下、保存処理等は変更なし（メイン処理で引数を渡すように修正） ---
...
# 各カテゴリの取得件数を指定
# 紙書籍: 4件, Kindle: 2件, Audible: 4件（2件から4件に変更）
